...
class TwinLiteNet(nn.Module):
    ...
    def forward(self, input):
        ...
        return self.dequant(out_da),self.dequant(out_ll)
...
if __name__ == '__main__':
    ...
    import torch
    import argparse
    for scale in [0.25,0.5,1.0,1.5,1.75,2.5]:
        print(scale)
        model = TwinLiteNet(scale).cuda()
        model = model
